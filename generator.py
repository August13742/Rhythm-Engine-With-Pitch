...
DIFF_WEIGHTS = {
    ...
    "INSANE": { "vocal": 1.10, "other": 1.00, "drums": 0.80, "bass": 0.80 }
}
...
SWITCH_PENALTY = 0.55  # Score multiplier when switching sources (range: 0.5-0.9, lower = discourages switching)
...
# # ==========================================
# #       V65: LOCAL GAIN CONTROL (AGC)
# # ==========================================

# # --- AUDIO ANALYSIS ---
# ANALYSIS_FMIN = librosa.note_to_hz('C1')  
# ANALYSIS_OCTAVES = 6  
# # Absolute noise floor. If raw audio is quieter than this, AGC won't touch it.
...
#     "drums": 3.5   
# }
# VOCAL_SMOOTH_FACTOR = 7  

# # --- AGC CONFIGURATION (NEW) ---
# # The window size for local normalization (in seconds).
# # 2.0s = Good for capturing phrase dynamics.
# AGC_WINDOW_SEC = 2.0 
...
# # --- PER-INSTRUMENT GATING ---
# # With AGC, we can use consistent gates again.
# SCORE_GATES = {
#     "EASY":   { "vocal": 0.35, "other": 0.35, "drums": 0.50, "bass": 0.50 },
#     "NORMAL": { "vocal": 0.30, "other": 0.30, "drums": 0.40, "bass": 0.40 },
#     "HARD":   { "vocal": 0.25, "other": 0.25, "drums": 0.30, "bass": 0.30 },
#     "INSANE": { "vocal": 0.15, "other": 0.15, "drums": 0.20, "bass": 0.20 }
# }

# # --- SOURCE SWITCHING BEHAVIOR ---
# COOLDOWN_BEATS = 0.65  
# SWITCH_PENALTY = 0.55  
# HYSTERESIS_FINAL_GATE = 0.2

# # --- CHORD DETECTION ---
# CHORD_TIME_WINDOW = 0.05  
# CHORD_SCORE_RATIO = 0.75  

# # --- RHYTHM QUANTIZATION ---
# GRID_PENALTIES = { 4: 1.0, 8: 1.0, 12: 2.0, 16: 1.2, 24: 2.5, 32: 3.0 }  
# SNAP_BLEND_RATIO = 0.95  
# MAX_SNAP_ERROR = 0.05  
...
# # --- GAMEPLAY DENSITY CONTROL ---
# MIN_NOTE_OVERRIDE_RATIO = 1.5  
# NON_MELODIC_BUDGET_THRESHOLD = 0.6  
# CHORD_SAME_TIME_WINDOW = 0.01  
# LANE_CROWDING_WINDOW = 0.01  

# # --- ENVELOPE NORMALIZATION ---
# NORM_PERCENTILE = 95  

# # --- HOLD NOTE DURATIONS ---
# BASS_HOLD_THRESHOLD = 0.8  
# BASS_HOLD_DURATION = 0.5  
# VOCAL_HOLD_THRESHOLD = 0.9  
# VOCAL_HOLD_DURATION = 0.4  
# OTHER_HOLD_THRESHOLD = 0.85  
# OTHER_HOLD_DURATION = 0.5   
...
# # --- PITCH-TO-LANE MAPPING ---
# DRUM_STRIKE_THRESHOLD = 0.7  
# BASS_PITCH_MIN = 36      
# BASS_PITCH_RANGE = 24    
# MELODIC_PITCH_MIN = 48   
# MELODIC_PITCH_MAX = 84   
...
#         # --- ENVELOPE PROCESSING WITH AGC ---
#         print("[GEN] Calculating AGC Envelopes...")
#         self.env_drum = self._get_env(self.y_drum_perc, use_agc=True) 
#         self.env_voc = self._get_env(self.y_voc_harm, smooth_factor=VOCAL_SMOOTH_FACTOR, use_agc=True) 
#         self.env_bass = self._get_env(self.y_bass_harm, use_agc=True)
        
#         # "OTHER" Special Handling: Transient Isolation + AGC
#         raw_env_oth = self._get_env(self.y_oth_harm, use_agc=False) # Get raw first
#         body_env_oth = scipy.signal.medfilt(raw_env_oth, kernel_size=15)
#         transient_oth = np.clip(raw_env_oth - (body_env_oth * 0.6), 0, 1) # 0.6 aggressiveness
#         # Now apply AGC to the transients only
#         self.env_oth = self._apply_agc(transient_oth)

#         self.data = self._analyze()

#     def _get_env(self, y, smooth_factor=0, use_agc=False):
#         # 1. Basic Onset Strength
#         env = librosa.onset.onset_strength(y=y, sr=self.sr)
        
#         # 2. Normalize 0-1 Global first
#         if env.max() > 0: env /= env.max()
        
#         # 3. Apply AGC (Local Gain)
#         if use_agc:
#             env = self._apply_agc(env)
...
#     def _apply_agc(self, env):
#         """
#         Automatic Gain Control:
#         Boosts quiet sections, maintains loud sections.
#         Ignores sections below SILENCE_THRESHOLD.
#         """
#         # Window size in frames
#         # librosa default hop_length is 512, sr=44100 -> ~86 frames per second
#         window_size = int(AGC_WINDOW_SEC * (self.sr / 512))
        
#         # Calculate Local Max (Dilation)
#         local_max = scipy.ndimage.maximum_filter1d(env, size=window_size)
        
#         # Avoid division by zero and boost noise
#         # We clamp the local_max to be at least SILENCE_THRESHOLD.
#         # This means signals below SILENCE_THRESHOLD won't get boosted to 1.0, they will stay small.
#         local_max = np.maximum(local_max, SILENCE_THRESHOLD)
        
#         # Apply Gain
#         return env / local_max
...
#             # Simple Static Gate (AGC makes this reliable now)
#             gate = gates.get(source_name, 0.2)
